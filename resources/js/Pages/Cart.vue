       ...
       <h5 v-if="total == '0,00'" class="mt-5 text-white text-center">{{$t('Panier vide')}}</h5>
        <ul class="list-group my-3 text-center d-flex justify-content-center">
           <li class="mb-2 bg-light p-2 position-relative" v-for="product in products" :key="product.id">
            <button type="button" class="m-auto list-group-item list-group-item-action col-12">{{product.name}} - {{ $t("quantité") }}: {{product.quantity}}  - {{ $t("prix") }}:  {{product.price.toFixed(2)}}€</button>
               ...
               <button v-if="product.quantity > 1" @click="remove(product.id)" class="btn btn-warning mt-2 btn-sm col-2">-</button>
               <button v-if="product.quantity == 1" @click="remove(product.id)" class="btn btn-danger mt-2 btn-sm col-2"><i class="fas fa-trash"></i></button>
